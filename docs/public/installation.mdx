...
- Detect your installed IDEs (Claude Code, Cursor, Gemini CLI, Windsurf, OpenCode, Codex CLI, Devin CLI) and let you multi-select which ones to wire up
...
Install an independent Devin CLI integration with:

```bash
npx claude-mem install --ide devin-cli
```

This copies the plugin to `~/.config/devin/plugins/claude-mem`, writes hooks and the MCP server into `~/.config/devin/config.json`, and sets `CLAUDE_CONFIG_DIR=~/.config/devin` for claude-mem hook execution. It does not require a Claude Code plugin installation.
...
- **Claude Code** or another supported IDE (Cursor, Gemini CLI, Windsurf, OpenCode, Codex CLI, Devin CLI, OpenClaw)
